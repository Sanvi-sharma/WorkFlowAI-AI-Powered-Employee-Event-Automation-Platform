# Setup Guide -- Windows Environment for n8n (Localhost)

## Objective

Bring up a local n8n instance on Windows via Docker Compose, running on
WSL2/Docker Desktop, and confirm it is reachable at `localhost:5678`.
...
-   Project workspace created (`C:\Projects\Automation-Lab\n8n\`)

------------------------------------------------------------------------

# 1. Create the `.env` File

Inside `C:\Projects\Automation-Lab\n8n\`, create `.env`:
...
Open PowerShell in `C:\Projects\Automation-Lab\n8n\`:
...
Restart the container and confirm workflows/credentials survive:
...
Once n8n is running, proceed to `ollama_setup_windows.md` to add a local
LLM backend for AI Agent nodes.